...
import { useAcademicTermById } from '../hooks/useAcademicTermById';
...
export const UpdateAcademicTerm = () => {
  ...
  const currentIndex = sortedTerms.findIndex((t) => t.id === Number(termId));

  const prevTerm = sortedTerms[currentIndex - 1];
  const nextTerm = sortedTerms[currentIndex + 1];

  const minDate = prevTerm
    ? new Date(prevTerm.endDate)
    : academicYear?.startDate
      ? new Date(academicYear.startDate)
      : undefined;

  const maxDate = nextTerm
    ? new Date(nextTerm.startDate)
  ...
  const { academicTerm } = useAcademicTermById(termId);
  ...
  useEffect(() => {
    form.reset({
      startDate: academicTerm?.startDate
        ? new Date(academicTerm?.startDate)
        : undefined,
      endDate: academicTerm?.endDate
        ? new Date(academicTerm?.endDate)
        : undefined,
      academicYearId: yearId,
    });
  }, [termId, academicTerm]);
  ...
  return (
    <div>
      <AcademicTermForm
        ...
        maxDate={maxDate}
        form={form}
        status={academicTerm?.academicTermStatus}
        onSubmit={onSubmit}
